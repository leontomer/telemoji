...
export function DetectionVideo({ videoRef }) {
    ...
    useEffect(() => {
        (async function () {
            try {
                let model = await tf.loadLayersModel(`${process.env.PUBLIC_URL}/facerecog/model.json`);
    ...
    const predict = (data) => {
        ...
        if (myModel && data) {
            try {
                let classNames = ['angry', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'suprise'];
    ...
    return (
        <div >
            <video ref={videoRef} autoPlay muted onPlay={handleVideoOnPlay} height={480} width={480} />
            <canvas ref={canvasRef} height={480} width={480} />
        </div>
    );
}
